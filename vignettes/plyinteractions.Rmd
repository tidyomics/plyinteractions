...
# Pinned `GInteractions` and anchored `GInteractions`
...
The `GInteractions` **must** be "pinned" (using `r pin_by()`) in order 
to specify which set of anchors should be affected by `plyranges` functions. 

```{r}
gi

gi |> pin_by("1") |> stretch(10)

gi |> pin_by("1") |> shift_right(20)

gi |> pin_by("1") |> flank_right(20)
```
...
```{r}
gi |> pin_by("1") |> anchor_start() |> stretch(10)

gi |> pin_by("1") |> anchor_center() |> stretch(10)
```
